...
class lstm_model(nn.Module):
    def __init__(self,input_dim,hidden_size,output_size,num_layers) -> None:
        ...
        self.conv1 = nn.Conv1d(input_dim,hidden_size,1,padding='same')
        self.pool1 = nn.AvgPool1d(1)
        self.conv2 = nn.Conv1d(hidden_size,hidden_size,3,padding='same')
        self.pool2 = nn.AvgPool1d(2)
        self.lstm = nn.LSTM(input_size=hidden_size,hidden_size=hidden_size, num_layers = num_layers,dropout=0.01)
    ...
    def forward(self,x):
        ...
        # convert x to shape batch_size X input_size X seq_len
        x = x.transpose(1,2)
        x = self.conv1(x)
        x = self.relu(x)
        x = self.tanh(x)
...
if __name__ == '__main__':
    ...
    model = lstm_model(input_dim=trn_x.shape[2],hidden_size=60,output_size=10,num_layers=3)
    train(model,train_dataloader,val_dataloader,80,batch_size)
    pkl.dump(model,open("../Model/cnn-lstm.pkl","wb"))  
